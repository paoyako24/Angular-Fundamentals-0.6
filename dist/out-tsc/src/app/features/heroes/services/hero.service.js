...
let HeroService = class HeroService extends HeroServiceAbstract {
    ...
    add(hero) {
        return this.#httpClient.post(this.API_ENDPOINT, hero).pipe(tap(newHero => this.#heroesSignal.update((currentHeroes) => [...currentHeroes, newHero])), catchError((error) => {
            console.error('Failed to add an hero', error);
            return throwError(() => error);
        }));
    }
    update(heroToUpdate) {
        return this.#httpClient.put(`${this.API_ENDPOINT}/${heroToUpdate.id}`, heroToUpdate).pipe(tap(updatedHero => this.#heroesSignal.update(currentHeroes => currentHeroes.map(heroe => heroe.id === updatedHero.id ? updatedHero : heroe))), catchError((error) => {
    ...
    findAll({ page, limit } = { page: 1, limit: 600 }) {
        return this.#httpClient.get(`${this.API_ENDPOINT}?_page=${page}&_limit=${limit}`).pipe(tap(result => this.#heroesSignal.set(result.heroes)));
    }
    findOne(id) {
